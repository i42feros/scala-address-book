...
import domain.questions.{CountBySex, OldestHuman, QuestionAnswer}
...
class HumanQuestionsService(filePath: String) extends ReplyQuestionsService[Human](PersonFileParser)(filePath) {
  override val questions: Seq[_ <: QuestionAnswer[Human,_ <: Any]] = Seq(CountBySex(Sex.Male), OldestHuman)
}
